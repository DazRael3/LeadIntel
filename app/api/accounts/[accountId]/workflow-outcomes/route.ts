...
export const GET = withApiGuard(
  async (request: NextRequest, { requestId, userId }) => {
    ...
    try {
      ...
      const link = await buildWorkflowOutcomeLinkage({ supabase, workspaceId: ws.id, accountId, windowDays })

      await logProductEvent({
        userId: user.id,
        eventName: 'workflow_outcome_link_viewed',
        eventProps: { workspaceId: ws.id, accountId, windowDays },
      })

      return ok(link, undefined, bridge, requestId)
    } catch (e) {
      return asHttpError(e, '/api/accounts/[accountId]/workflow-outcomes', userId, bridge, requestId)
    }
  },
