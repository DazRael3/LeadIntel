...
function extractAccountIdFromPath(pathname: string): string | null {
  // /api/accounts/[accountId]/actions/crm
    ...
    try {
      ...
      const queueItem = await createActionQueueItem({
        ...
        payloadMeta: {
          kind: payload.kind,
          window: parsed.data.window ?? '30d',
          mode: payload.crm.mode,
          companyName,
          taskTitle: payload.crm.taskTitle,
          noteTitle: payload.crm.noteTitle,
      ...
      await logProductEvent({
        userId: user.id,
        eventName: 'crm_handoff_prepared',
        eventProps: {
          workspaceId: workspace.id,
          accountId,
          mode: payload.crm.mode,
      ...
      return asHttpError(error, '/api/accounts/[accountId]/actions/crm', userId, bridge, requestId)
    }
  },
