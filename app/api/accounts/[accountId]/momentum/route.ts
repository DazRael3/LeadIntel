...
function extractAccountIdFromPath(pathname: string): string | null {
  // /api/accounts/[accountId]/momentum
...
export const GET = withApiGuard(
  async (request: NextRequest, { requestId, userId, query }) => {
    ...
    try {
      ...
      return ok(explainability.momentum, undefined, bridge, requestId)
    } catch (err) {
      return asHttpError(err, '/api/accounts/[accountId]/momentum', userId, bridge, requestId)
    }
  },
