...
export const GET = withApiGuard(
  async (request: NextRequest, { requestId, userId, query }) => {
    ...
    try {
      ...
      return ok({ personas: explainability.people.personas, buyingGroup: explainability.people.buyingGroup }, undefined, bridge, requestId)
    } catch (err) {
      return asHttpError(err, '/api/accounts/[accountId]/personas', userId, bridge, requestId)
    }
  },
