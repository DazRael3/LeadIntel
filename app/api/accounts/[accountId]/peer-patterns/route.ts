...
export const GET = withApiGuard(async (request: NextRequest, { requestId, userId }) => {
  ...
  try {
    ...
    const accountId = seg.length >= 4 ? seg[2] : null
    if (!accountId) return fail(ErrorCode.VALIDATION_ERROR, 'Account id required', undefined, undefined, bridge, requestId)
    ...
    const insight = await getAccountPeerPatternInsight({
      supabase,
      userId: user.id,
      accountId,
      window: parsed.data.window,
      crossWorkspaceEnabled: policies.benchmarks.crossWorkspaceInsightsEnabled,
    })

    await logProductEvent({ userId: user.id, eventName: 'peer_pattern_viewed', eventProps: { workspaceId: ws.id, accountId, window: parsed.data.window } })

    return ok({ workspaceId: ws.id, role: membership.role, insight }, undefined, bridge, requestId)
  } catch (e) {
    return asHttpError(e, '/api/accounts/[accountId]/peer-patterns', userId, bridge, requestId)
  }
})
