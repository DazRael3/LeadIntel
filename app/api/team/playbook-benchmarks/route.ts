...
import { getPlaybookBenchmarks } from '@/lib/services/playbook-benchmarks'
...
export const GET = withApiGuard(async (request: NextRequest, { requestId, userId }) => {
  ...
  try {
    ...
    const res = await getPlaybookBenchmarks({
      supabase,
      workspaceId: ws.id,
    ...
    await logProductEvent({ userId: user.id, eventName: 'playbook_benchmark_viewed', eventProps: { workspaceId: ws.id, windowDays: parsed.data.windowDays } })

    return ok({ workspaceId: ws.id, role: membership.role, windowDays: parsed.data.windowDays, rows: res.rows }, undefined, bridge, requestId)
  } catch (e) {
    return asHttpError(e, '/api/team/playbook-benchmarks', userId, bridge, requestId)
  }
})
