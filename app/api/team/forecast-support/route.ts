...
import { getWorkspaceForecastSupport } from '@/lib/revenue/engine'
...
export const GET = withApiGuard(async (request: NextRequest, { requestId, userId }) => {
  ...
  try {
    ...
    const summary = await getWorkspaceForecastSupport({ supabase, userId: user.id, workspaceId: ws.id, windowDays: 30 })

    await logProductEvent({ userId: user.id, eventName: 'forecast_support_viewed', eventProps: { workspaceId: ws.id } })

    return ok({ workspace: { id: ws.id, name: ws.name }, role: membership.role, summary }, undefined, bridge, requestId)
  } catch (e) {
    return asHttpError(e, '/api/team/forecast-support', userId, bridge, requestId)
  }
})
