...
export const GET = withApiGuard(async (request: NextRequest, { requestId, userId }) => {
  ...
  try {
    ...
    await logProductEvent({
      userId: user.id,
      eventName: 'benchmark_board_viewed',
      eventProps: { workspaceId: ws.id, windowDays: parsed.data.windowDays },
    })

    return ok({ workspace: { id: ws.id, name: ws.name }, role: membership.role, windowDays: parsed.data.windowDays, metrics: res.metrics }, undefined, bridge, requestId)
  } catch (e) {
    return asHttpError(e, '/api/team/benchmarks', userId, bridge, requestId)
  }
})
