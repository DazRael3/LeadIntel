...
import { serializeActionQueueItem, type ActionQueueRow } from '@/lib/platform-api/serializers/action-queue'
...
const QuerySchema = z.object({
  ...
  status: z.enum(['ready', 'queued', 'processing', 'delivered', 'failed', 'blocked', 'manual_review']).optional(),
})
...
export async function GET(request: NextRequest) {
  ...
  const authed = await withPlatformAuth({ request, requestId, requiredScopes: ['action_queue.read'] })
  ...
  try {
    ...
    let q = admin
      ...
      .select('id, workspace_id, lead_id, action_type, status, destination_type, destination_id, reason, error, created_at, updated_at, payload_meta')
    ...
    const { data } = await q
    const rows = (data ?? []) as unknown as ActionQueueRow[]
    const page = rows.slice(0, parsed.data.limit)
    const next = rows.length > parsed.data.limit ? rows[parsed.data.limit] ?? null : null

    const items = page.map((r) => serializeActionQueueItem(r))
    const response: ListResponse<typeof items[number]> = {
      items,
      pagination: { nextCursor: next ? encodeCursor({ t: next.created_at, id: next.id }) : null },
    }
