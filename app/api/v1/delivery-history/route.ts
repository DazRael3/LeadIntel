...
import { serializeDelivery, type DeliveryRow } from '@/lib/platform-api/serializers/delivery'
...
const QuerySchema = z.object({
  ...
  status: z.enum(['queued', 'processing', 'delivered', 'failed']).optional(),
})
...
export async function GET(request: NextRequest) {
  ...
  const authed = await withPlatformAuth({ request, requestId, requiredScopes: ['delivery.read'] })
  ...
  try {
    ...
    let q = admin
      ...
      .select('id, workspace_id, queue_item_id, action_type, destination_type, destination_id, status, webhook_delivery_id, export_job_id, error, meta, created_at')
    ...
    const items = page.map((r) => serializeDelivery(r))
