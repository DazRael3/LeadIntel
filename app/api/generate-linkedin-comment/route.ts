...
const LINKEDIN_FALLBACK_COMMENT = 'Congratulations on the recent momentum. Wishing your team continued success as you build on it.'
  ...
  async (request: NextRequest, { body, userId, requestId }) => {
    ...
    try {
      ...
      const aiResult = await generateWithProviderRouter({
        task: 'outreach_draft',
        system: `You are a professional LinkedIn engagement expert. Write authentic, helpful LinkedIn comments that build rapport.
Rules:
...
- If relevant, subtly connect to what the company does`,
        prompt: `Write a LinkedIn comment for ${companyName}'s recent activity: ${triggerEvent}
${(userSettings as { whatYouSell?: string } | undefined)?.whatYouSell ? `Context: We sell ${(userSettings as { whatYouSell?: string }).whatYouSell} but don't mention this directly.` : ''}`,
        ...
        maxTokens: 150,
        metadata: {
          route: '/api/generate-linkedin-comment',
          userId,
        },
      })

      const comment = aiResult.ok
        ? aiResult.text.trim()
        : `Congratulations on ${triggerEvent}! ${LINKEDIN_FALLBACK_COMMENT}`
