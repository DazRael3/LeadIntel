...
const FOLLOW_UP_OPENERS = [
  'Thanks for requesting a LeadIntel demo.',
  'Your LeadIntel demo request is in.',
  'We received your LeadIntel request and queued next steps.',
] as const
...
function getIntentLabel(intent: 'demo' | 'pricing_question' | 'trial_help' | 'general'): string {
  if (intent === 'pricing_question') return 'pricing question'
  if (intent === 'trial_help') return 'trial help request'
  if (intent === 'general') return 'general request'
  return 'demo request'
}

function getFollowUpSubject(intent: 'demo' | 'pricing_question' | 'trial_help' | 'general'): string {
  if (intent === 'pricing_question') return 'LeadIntel received your pricing question'
  if (intent === 'trial_help') return 'LeadIntel received your trial help request'
  if (intent === 'general') return 'LeadIntel received your request'
  return 'Your LeadIntel demo outline'
}
...
export function renderLeadCaptureConfirmationEmail(args: {
  ...
  const subject = appendSubjectTag(getFollowUpSubject(args.intent), args.requestId)
  const intentLabel = getIntentLabel(args.intent)
  ...
  const html = `<!doctype html><html><body style="margin:0;padding:24px;background:#050a14;color:#e5e7eb;font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial;">
    <div style="max-width:640px;margin:0 auto;">
      ${header}
      <h1 style="margin:10px 0 0 0;font-size:20px;color:#e5e7eb;">${escapeHtml(getFollowUpSubject(args.intent))}</h1>
      ...
      <div style="margin-top:12px;border:1px solid rgba(148,163,184,0.25);border-radius:10px;padding:12px;background:#0b1220;">
        ...
        <div style="margin-top:4px;font-size:13px;color:#cbd5e1;"><strong>Route:</strong> ${escapeHtml(args.route)}</div>
      </div>
      ${demoPlanHtml}
